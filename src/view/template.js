...
Candy.View.Template = (function(self){
	...
	self.Chat = {
		...
		tab: '<li class="roomtype-{{roomType}}" data-roomjid="{{roomJid}}" data-roomtype="{{roomType}}"><a href="#" class="label">{{#privateUserChat}}@{{/privateUserChat}}{{name}}</a><a href="#" class="transition"></a><small class="unread"></small></li>',
	...
	self.Message = {
		...
		item: '<li><span class="message-time">{{time}}</span><span class="message-user">{{displayName}}</span><span class="message-message">{{{message}}}</span></li>'
	};
